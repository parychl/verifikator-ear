...
Důvěra se odvozuje z EU Trusted Listu (TSPTrustManager) a kvalifikovanost
certifikátů i časových razítek se posuzuje QualificationAssessorem nad
validační cestou — stejná logika, jakou pyHanko používá ve svém AdES flow
...
class ValidacniZdroje:
    """Sdílené zdroje důvěry pro validaci (trusted list + posuzovatel kvalifikovanosti)."""
...
def sestav_validacni_zdroje(registry: TSPRegistry) -> ValidacniZdroje:
...
def _je_kvalifikovany(zdroje: ValidacniZdroje, validation_path) -> bool:
    """Posouzení kvalifikovanosti dle trusted listu (zrcadlí pyHanko _qualification_analysis)."""
    if validation_path is None:
        return False
    try:
        vysledek = zdroje.assessor.check_entity_cert_qualified(validation_path)
...
def _info_razitka(zdroje: ValidacniZdroje, ts_status) -> InfoRazitka:
    ...
    razitko.kvalifikovane = razitko.podpis_platny and _je_kvalifikovany(
        zdroje, getattr(ts_status, "validation_path", None)
    )
    return razitko
...
def _vypln_stav(info: InfoPodpisu, status, zdroje: ValidacniZdroje) -> None:
    ...
    info.kvalifikovany_cert = _je_kvalifikovany(
        zdroje, getattr(status, "validation_path", None)
    )
...
def zvaliduj_docasove_razitko(
    ...
    info = _spolecne_udaje(sig)
    try:
        status = validate_pdf_timestamp(sig, zdroje.novy_kontext())
    except Exception as e:
        log.warning(
